    ...
    if (user_id != undefined) {
        getUserData(user_id).then(user_data => {
            getUserRoles(user_id).then(roles => {
                ...
                user_div.appendChild(element);
                // Genres
                document.getElementById('genres-heading').innerHTML = '<b>Top genres</b>';
                if (user_data.statistics?.top_genres != undefined) {
                    ...
                    user_data.statistics.top_genres.forEach(entry => {
                        ...
                        element.innerHTML = `${keyToStr(entry.genre)} (${entry.listens})`;
                        span.appendChild(element);
                        // Append span
                ...
                // Artists
                document.getElementById('artists-heading').innerHTML = '<b>Top artists</b>';
                if (user_data.statistics?.top_artists != undefined) {
                    ...
                    user_data.statistics.top_artists.forEach(entry => {
                        ...
                        element = document.createElement('p');
                        element.classList.add('text');
                        element.innerHTML = `<a onclick="gotoArtist('${keyToStr(entry.artist)}')"><u>${keyToStr(entry.artist)}</u></a>  (${entry.listens})`;
                        span.appendChild(element);
                        // Append span
                        artists_div.appendChild(span);
                    })
                }
                // Tracks
                document.getElementById('tracks-heading').innerHTML = '<b>Top tracks</b>';
                if (user_data.statistics?.top_tracks != undefined) {
                    ...
                    user_data.statistics.top_tracks.forEach(entry => {
                        ...
                        element = document.createElement('p');
                        element.classList.add('text');
                        element.innerHTML = `<a onclick="gotoArtist('${keyToStr(entry.artist)}')"><u>${keyToStr(entry.artist)}</u></a> - ${keyToStr(entry.track)} (${entry.listens})`
                        span.appendChild(element);
                        // Append span
                        tracks_div.appendChild(span);
                    })
                }
            });
        });
