            ...
            getUserRoles(user_id).then(roles => {
                ...
                if (user_data.statistics?.top_artists != undefined) {
                    ...
                    user_data.statistics.top_artists.forEach(entry => {
                        ...
                        element = document.createElement('p');
                        element.classList.add('text');
                        element.innerHTML = `<a onclick="gotoArtist('${entry.artist}')"><u>${keyToStr(entry.artist)}</u></a>  (${entry.listens})`;
                        span.appendChild(element);
                        // Append span
                ...
                if (user_data.statistics?.top_tracks != undefined) {
                    ...
                    user_data.statistics.top_tracks.forEach(entry => {
                        ...
                        element = document.createElement('p');
                        element.classList.add('text');
                        element.innerHTML = `<a onclick="gotoArtist('${entry.artist}')"><u>${keyToStr(entry.artist)}</u></a> - ${keyToStr(entry.track)} (${entry.listens})`
                        span.appendChild(element);
                        // Append span
