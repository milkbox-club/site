...
function renderTags(tags) {
    var links = [];
    for (var tag in tags) {
        links.push(`<a onclick="showTagPosts('${tags[tag]}')"><u>${tags[tag]}</u></a>`);
    }
    return `<p>Tags:&nbsp;${links.join('&#32;&bull;&#32')}</p>`
}
...
function showTagPosts(tag) {
    showPostFunction = function() { showTagPosts(tag) };
    const url = `/api/getTagPosts?application_id=${application_id}&tag=${tag}`;
    console.log('api_url=', url);
    document.getElementById('filter-heading').innerHTML = `<b>"${tag}" posts</b>`;
